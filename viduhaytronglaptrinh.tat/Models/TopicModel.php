...
class TopicModel extends DB {
  function getAllPost(){
    if($this->conn){
      ...
      $sql1 = "SELECT topic.`id_topic`, `topic_name`, `post_views` , `slComment`, `slLike` FROM topic LEFT JOIN (select `id_topic`, `post_views` , COUNT(`id`) as `slComment`, COUNT(`id_like`) as `slLike` from (select D.`id_post`, `id_topic`, `post_views` , `id`, `id_like` FROM (select  A.`id_post`, `id_topic`, `post_views` , comments.`id_post` as `id` from (select `id_post`, `id_topic`, `post_title`, `post_content`, `post_date_created`, `post_views` FROM post) as A LEFT JOIN comments on A.`id_post` = comments.`id_post`) as D LEFT JOIN likes on D.`id_post` = likes.`id_post`) as B GROUP BY `id_post`) as C on C.`id_topic` = topic.`id_topic`";
      $kq = $this->fetch_assoc($sql1, 0);
      $this->disConnect();
      return $kq;
    }
  }
  ...
  function getNameTopic($id){
    if($this->conn){
      $sql = "select `topic_name` from topic where `id_topic` = '{$id}'";
      $kq = $this->fetch_assoc($sql, 1);
      $this->disConnect();
      return $kq;
    }
  }
  function isNameTopic($name){
    if($this->conn){
      $sql = "select count(`id_topic`) as sl from topic where `topic_name` = N'{$name}'";
      $kq = $this->fetch_assoc($sql, 1);
      $this->disConnect();
      return $kq;
    }
  ...
  function createTopic($name){
    if($this->conn){
      $sql = "INSERT INTO topic (id_topic, topic_name) VALUES (NULL, '{$name}')";
      $kq = $this->query($sql);
      $this->disConnect();
      return $kq;
    }
